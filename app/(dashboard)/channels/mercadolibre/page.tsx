...
export default function MercadoLibrePage() {
  ...
  const fetchData = async () => {
    try {
      // 1. Traer productos
      const itemsRes = await fetch(`https://oauth.goqconsultant.com/integrations/mercadolibre/items?tenant_id=${tenant_id}&channel_id=${channel_id}`);
      const itemsData = await itemsRes.json();
      setItems(itemsData);

      // 2. Traer estado de la importación
      const statusRes = await fetch(`https://oauth.goqconsultant.com/integrations/mercadolibre/import/latest?tenant_id=${tenant_id}&channel_id=${channel_id}`);
